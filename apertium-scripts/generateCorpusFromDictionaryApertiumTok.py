...
for i in range(len(translations)):
	...
	if not alignments:
		...
		if not duplicateToBeDetermined or withLearnedRules:
			...
			#since we will use the apertium tokenizer, we do not need these alignments
			fals.write(u" ||| ".join([debugInfo.alignmentsPreTransfer.toString(),debugInfo.alignmentsTransfer.toString()])+u"\n") 
		else:
			#debug
			...
			#fals.write(finalAlignments.toStringMinusOne()+u"\n")
			fals.write(u" ||| ".join([debugInfo.alignmentsPreTransfer.toString(),debugInfo.alignmentsTransfer.toString(),debugInfo.alignmentsInterchunk.toString(),debugInfo.alignmentsPostchunk.toString()])+u"\n")
