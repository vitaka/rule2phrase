...
inputt1x=False
postTransfer=False
additionalInfoFileName=None

try:
	opts, args = getopt.getopt(sys.argv[1:], "ib:ds:t:m:o:a:hwp", ["inputt1x","duplicateToBeDetemrined:","debug","sourcelang=","targetlang=","mwlexicalforms=","outputt1x=","additional=","help","writeDebugInfo", "postTransfer" ])
...
for o, a in opts:
	if o in ("-h", "--help"):
		...
		writeDebugInfo=True
	elif o in ("-a","--additional"):
		additionalInfoFileName=a
...
if additionalInfoFileName != None:
	fadditional=open(additionalInfoFileName,"w")
...
if not duplicateToBeDetemrined and not inputt1x:
	...
	if debugOn:
		...
		for t in translations:
			...
			# we don't need generation and postgeneration alignments: we will use Apertium analysis instead
			...
			print debugInfo.translation.encode('utf-8')+" ||| "+debugInfo.alignmentsTransfer.toString().encode('utf-8')
			if additionalInfoFileName != None:
				fadditional.write(debugInfo.debugTransfer.encode("utf-8")+"\n")
	else:
		for t in translations:
